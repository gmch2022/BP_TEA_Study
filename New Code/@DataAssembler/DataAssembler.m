...
%----------------------------------------------------------------------------
        ...
        [intervals]                  = tempSplitInterval(obj, param)                              % Spliting the interval in BP segment,
        ...
        function [syncInfo] = tempConfirmSyncSegment(~,SynchronizeWave)
            ...
            minLength = min(length(rising_edges), length(falling_edges));

            rising_edges = rising_edges(1:minLength);
            falling_edges = falling_edges(1:minLength);
